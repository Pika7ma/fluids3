...
__global__ void insertFineParticles(bufList buf, int pnum) {
    ...
    int nadj = (1 * simData.gridRes.z + 1)*simData.gridRes.x + 1;	// Center of neighbor cells
    uint gc = buf.mgcell[i];	// Grid index of central coarse particle
    if (gc == GRID_UNDEF) return;
    gc -= nadj;

    float3 pos = buf.mpos[i];	// Central coarse particle position

    std::vector<float3> fineParticlePositions;	// Store available fine particles

    pos -= float3(gridDelta);	// First sampling position of central coarse particle
    int sampleNum = 20;	// Sample number in one demension
    float sampleDelta = 2.0 / sampleNum;	// Sample distance interval

                                            // For all neighbor grid
    for (int c = 0; c < simData.gridAdjCnt; c++) {
        ...
        float dsq;	// 1D distance^2

                    // Sample sampleNum^3 points uniformly around central coarse particle
        for (int i = 0; i <= sampleNum; ++i) {
            for (int j = 0; j <= sampleNum; ++j) {
                for (int k = 0; k <= sampleNum; ++k) {

                    float3 finePos = pos += float3(i*sampleDelta*gridDelta, j*sampleDelta*gridDelta, k*sampleDelta*gridDelta);	// Try sample

                    for (int cndx = cfirst; cndx < clast; ++cndx) {
                        dist = finePos - buf.mpos[buf.mgrid[cndx]];
                        dsq = (dist.x*dist.x + dist.y*dist.y + dist.z*dist.z);
                        if (dsq < gridDelta * gridDelta) break;	// Dont sample at this point (lambda_c = grid size = gridDelta)

                        fineParticlePositions.push_back(finePos);	// Do sample

                                                                    // Operations that insert fine particle into storage
                    }
                }
            }
        }
